...
struct TopicCommentsHeaderView: View {
    ...
    var body: some View {
        HStack() {
            if authorName == topicAuthorName {
                ...
                ).background(style.topicAuthorCommentBackgroundColor)
            } else {
                Text(
                    authorName
                ).font(
    ...
    struct Style {
        ...
        let topicAuthorCommentBackgroundColor = Color(red: 1.00, green: 0.90, blue: 0.30, opacity: 1.00)
